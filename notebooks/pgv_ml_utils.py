...
def k_fold_results(train_set, param):
    ...
    for i in np.arange(param['n_folds']):
        ...
        clf = RandomForestClassifier(n_estimators=param['n_estimators'], max_depth=param['max_depth'],
                                     class_weight=param['class_wt'], random_state=10, n_jobs=-1).fit(X_train, y_train)
...
def read_vel(array):
    ...
    vel_df = pd.read_csv(fpath, header=None, sep=' ', usecols=[1, 2, 3, 4, 5],
                         names=['gpst', 'dndt', 'dedt', 'dudt', 'clk'])
...
def gpsleapsec(gpssec):
    ...
    leaptimes = np.array(
        [46828800, 78364801, 109900802, 173059203, 252028804, 315187205, 346723206, 393984007, 425520008, 457056009,
         504489610, 551750411, 599184012, 820108813, 914803214, 1025136015, 1119744016, 1167264017])
...
def pgv_threshold(ds, quantile_perc):
    """
            ...
            Parameters
            ----------
            ds is pandas df of 3 component velocities
            quantile_perc is ncx2 quantile

            :return:
               thresh is upper threshold value
            """
    nc = np.mean(ds['dedt']) ** 2 + np.mean(ds['dndt']) ** 2 + np.mean(ds['dudt']) ** 2
    df = 3

    df, nc, loc, scale = ncx2.fit(ds['gv2'].values, fdf=df, fnc=nc)
    up_val = ncx2.ppf(quantile_perc, df, nc, scale=scale, loc=loc)
    thresh = np.sqrt(up_val)
...
def get_features(dataframe, obs):
    """
        ...
        Parameters
        ----------
        dataframe is directional pandas series: series
        obs is directional component : str

        :return:
           feature_ds dataframe of features
        """
    ...
    feature_ds = pd.DataFrame({'time': [dataframe.index[-1]], obs + '_max_val': [dataframe.nlargest(2)[0]],
                               ...
                               obs + '_min_val': [dataframe.nsmallest(2)[0]],
                               obs + '_min2_val': [dataframe.nsmallest(2)[1]],
...
def rt_class(row, threshold, fs_params, clf, save_plot=False):
    """
        Function that runs RF model in 'real-time' 1/second

        Parameters
        ----------
        :param row: row of pandas df of station-event pair: series

        :param threshold: decision threshold hyperparameter: float

        :param fs_params: (additional hyperparmaters:dict

        :param clf: rf classifier model: array

        :param save_plot: bool

        :return:
            :firstd: time of first detection in rt mode
            :hypo: re-calculated hypocentral radius
    ...
    if save_plot:
        plot_rt(vel_df, site, eq, station, times_rt, y_rt_pred, hypo)
...
sta_coords_df = sta_coords_df.drop_duplicates(subset=sta_coords_df.columns[0])[
    [sta_coords_df.columns[0], sta_coords_df.columns[2], sta_coords_df.columns[3]]]
